...
// your code below!
let email = getInput(1)
let password = getInput(2)

if (isRegisteredUser(email) === true && passwordMatches(email,password) === true) {
  console.log("you're logged in")
}

if(isRegisteredUser(email) === true && passwordMatches(email,password) === false) {
  console.log("your password is incorrect")
}

if (isRegisteredUser(email) !== true && isValidEmail(email) === true && isValidPassword(password) === true) {
  console.log("You're signed up for an account!!")
}

if (isRegisteredUser(email) !== true) {
   if(isValidEmail(email) === false) {
    console.log("Your email is not valid, it must end with .prsvr@gmail.com or @perseverenow.org and contain at least 1 letter before.")
   } else if(isValidPassword(password) === false) {
    console.log("Your password is not valid it must contain 8 digits with at least one uppercase and one lowercase.")
   } else {
    console.log("none of these are valid responses.")
   }
}
